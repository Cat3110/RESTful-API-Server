...
handlers._users.post = function(data, callback) {
  // Ceck that all required fields are filled out
  ...
  if (firstName && lastName && phone && password && tosAgreement) {
    ...
    _data.read('users', phone, function(err, data) {
      if (err) {
        ...
        // User already exist
        callback(400, {'Error' : 'A user with than phone number is already exist'});
      }
    })
  } else {
                    ...
                    _data.delete('checks', checkId, function(err) {
                      ...
                      if(checksDeleted == checksToDelete) {
                        if(!deletionErrors) {
                          ...
                          callback(500, {'Error' : 'Errors encountered while attemptint to delete all of the users checks. '});
                        }
                      }
                    });
                  });
...
handlers._tokens.post = function(data, callback) {
  ...
  if(phone && password){
    ...
    _data.read('users', phone, function(err, userData){
      if(!err && userData) {
        // Hash the sent password, and compare with iser password
        var hashedPassword = helpers.hash(password);
...
// Containger for all the checks methods
handlers._checks = {};
...
handlers._checks.post = function(data, callback) {
  ...
  if (protocol && url && method && successCodes && timeoutSeconds) {
    ...
    _data.read('tokens', token, function(err, tokenData) {
      if(!err && tokenData) {
        ...
        _data.read('users', userPhone, function(err, userData) {
          if(!err && userData) {
            var userChecks = typeof(userData.checks) == 'object' && userData.checks instanceof Array ? userData.checks : [];
            // Verify that the user has less than the number of max-ckecks-per-user
            if(userChecks.length < config.maxChecks) {
              ...
              _data.create('checks', checkId, checkObject, function(err) {
                if (!err) {
                  // Add the ckeck id to the user's object
    ...
    callback(400, {'Error' : 'Missing required inputs, or input is invalid'});
  }
...
// Checks - put
// Required data: id
// Optional data: protocol, url, method, successCodes, timeoutSeconds (at least one of them must be send)
              ...
              if(!err) {
                ...
                _data.read('users', checkData.userPhone, function(err, userData) {
                  if(!err && userData) {
                    ...
                    callback(500, {'Error' : 'Could not find the user who created the check, so could not remove the check from the user\'s checks'});
                  }
                });
              } else {
